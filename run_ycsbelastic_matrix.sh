...
NUMA_CPUBIND=0
DB="elastic"
DB_PROPERTIES="./elastic/elastic.properties"
STATUS_INTERVAL=1
SLEEP_AFTER_LOAD=200
ENABLE_ASYNC_TEST=true
OUTPUT_DIR="./logs/ycsbelastic"
YCSB_BIN="./ycsbelastic"

usage() {
  cat <<'EOF'
Usage:
  ./run_ycsbelastic_matrix.sh [-p name=value]...

Example:
  ./run_ycsbelastic_matrix.sh -p elastic.write_buffer_size=134217728 -p elastic.max_open_files=5000
  ...
  for threads in "${THREADS_LIST[@]}"; do
    for limit_ops in "${LIMIT_OPS_LIST[@]}"; do
      for max_background_jobs in "${MAX_BACKGROUND_JOBS_LIST[@]}"; do
        run_id=$((run_id + 1))
        log_file="$OUTPUT_DIR/${workload_name}_t${threads}_ops${limit_ops}_bg${max_background_jobs}_rec${RECORDCOUNT}.log"
        ...
        cmd=(
          ...
          -p "elastic.min_compaction_threads=${max_background_jobs}"
        ...
        echo "[$run_id] Running workload=$workload_name threads=$threads limit.ops=$limit_ops elastic.min_compaction_threads=$max_background_jobs"
